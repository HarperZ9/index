...
def test_status_json_is_action_envelope(capsys):
    ...
    assert "router-job" in payload["native"]["commands"]
    assert {"index.router.job.start", "index.router.job.status",
            "index.router.job.result", "index.router.job.cancel",
            "index.router.job.resume"} <= set(payload["native"]["mcp_tools"])
