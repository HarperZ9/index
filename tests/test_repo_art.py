...
The truncation tests earn their place. Prose in a stage note or a card row is
wrapped to a fixed number of lines and the wrapper drops the rest, so an edited
sentence can lose its ending in the drawing while reading fine in the spec.

Everything here settles whether a drawing fits its columns and matches its
spec. Whether the card is TRUE of what index writes into a map is a different
question, and tests/test_gitmeta.py and tests/test_scan.py settle that one by
driving the code.
"""
...
def _card_text_that_overflows(card: dict) -> list[str]:
    """A card row holds a key and a value on one unwrapped line each, and a note
    wrapped to two. None of the three is clipped by the renderer, so an
