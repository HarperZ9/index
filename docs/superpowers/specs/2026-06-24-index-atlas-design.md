...
1. **Readable docs (perceptible):** a new zero-dep **minimal markdown→HTML renderer** (`knowledge/markdown.py`) supporting the common subset — ATX headings, paragraphs, unordered/ordered lists, fenced + inline code, bold/italic, links `[t](u)`, and **`[[wiki-links]]`** rendered as clickable spans (`<a data-atlas-target="…">`). Every text segment is HTML-escaped; URLs are attribute-escaped; no raw HTML passthrough (so untrusted doc content can't inject). Clicking a `[[link]]` navigates the graph (no page nav).
