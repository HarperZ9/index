...
`resolve_edges` gains a fallback: when a target name contains `/` and has no exact match in the index, try the longest exposed name that is a segment-aligned path prefix of the target. So an import of `github.com/org/repo/pkg` resolves to the repo whose module is `github.com/org/repo`. The fallback only fires when exact matching fails, only for slash-containing targets, and prefers the longest prefix, so it is safe for the existing resolvers. Python, Rust, and Java dependency names have no slashes, so they never enter the fallback. A JS scoped name like `@scope/pkg` does reach resolution with its slash, but no exposed name is a segment-aligned path-prefix of a two-segment `@scope/pkg` unless a repo exposes the bare scope `@scope`, so the fallback cannot fire for it. A unit test locks the behavior, including the scoped-JS case.
