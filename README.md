...
| **Symbol navigation (def / refs / impls)** | `index symbols QUERY` | Go-to-definition, find-references, and find-implementations for one symbol from the CLI (and JSON), each hop carrying `file:line` evidence; find-implementations resolves in-repo subclasses of a class and method overrides of a method, and an external or unbindable base is never guessed into an edge |
